...
kony.apps.ess.myLeaveTabUI.Reusable = kony.apps.ess.myLeaveTabUI.Reusable || {};
kony.apps.ess.myLeaveTabUI.Reusable.calendar = kony.apps.ess.myLeaveTabUI.Reusable.calendar || {};
kony.apps.ess.myLeaveTabUI.Reusable.calendar.HelperFunction = kony.apps.ess.myLeaveTabUI.Reusable.calendar.HelperFunction || {};
...
cell_focus_skin					- This is the default cell focus skin in the calander(the type of skin need to be specified was of FLEXCONTAINER).
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET = function (month_param, year_param, Flex_name, cell_skin, cell_focus_skin, calendarFlex_skin, monthFlex_skin, monthBTN_Skin_param, monthBTN_FocusSkin_param, weekLabel_skin, label_skin_BelongMonth, label_skin_NotBelongMonth, isValidMonthandYearforCalendarCallBackFunction, errorcallbackIsValidMonthandYearCallBackFunction, onswipeCallBackFunction, onTouchEndCallBackFuntion, callbackMonthChangeFunction_param, callbackRefershTotalcalendarFucntion_param) {
	...
	for (var i = 0, day = this.WeekStartDay; i < 7; i++, day++) {
		...
		var lblBasic = {
			...
			"text" : kony.apps.ess.myLeaveTabUI.Reusable.calendar.HelperFunction.retriveDayName(day).substr(0, 3)

		};
		var label_Week = new kony.ui.Label(lblBasic, {}, {});
	...
	for (var calendarIndex = 0; calendarIndex < (this.FLEXCOUNT); calendarIndex++) {
		...
		this[basicconfig_calendar.id].setGestureRecognizer(constants.GESTURE_TYPE_SWIPE, {
			...
			if (gestureInfo.swipeDirection == 2) {
				var shiftingmonthdetails = kony.apps.ess.myLeaveTabUI.Reusable.calendar.HelperFunction.retriveMonthDetails(this.month, this.year,  - 1);
		...
		for (var i = 0; i < 42; i++) {
			if (i % 7 === 0) {
				...
				basicconfig_cell.left = "0%";
				kony.print("the value of top and left in " + i + "iteration was " + basicconfig_cell.top + " and " + basicconfig_cell.left);
			} else {
				basicconfig_cell.left = (parseFloat(basicconfig_cell.left.replace("%", "")) + 14.29).toString() + "%";
			}

			basicconfig_cell.id = "FLEX_CELL" + calendarIndex + "INDEX" + i;
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.MonthDataConstructorIntializate = function () {

	for (var i = 0, diff = -1; i < this.MONTHHEADERCOUNT; i++, diff++) {
		var monthYearKeyPair = kony.apps.ess.myLeaveTabUI.Reusable.calendar.HelperFunction.retriveMonthDetails(this.month, this.year, diff)
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.updateMonthHeaders = function () {
	...
	for (var i = 0, diff = -1; i < this.MONTHHEADERCOUNT; i++, diff++) {
		...
		this[BtnHeaderId].text = kony.apps.ess.myLeaveTabUI.Reusable.calendar.HelperFunction.retriveMonthName(this.MonthData[i].MONTH).substr(0, 3) + " " + this.MonthData[i].YEAR.toString().substr(2, 2);
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.DataConstructorIntializate = function () {
	var middleFlex = parseInt((this.FLEXCOUNT - 1) / 2);

	for (var calendarIndex = middleFlex; calendarIndex < (this.FLEXCOUNT); calendarIndex++) {
		this.calendarDataIntialize(calendarIndex, kony.apps.ess.myLeaveTabUI.Reusable.calendar.HelperFunction.retriveMonthDetails(this.month, this.year, (calendarIndex - middleFlex)));
	}
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.calendarDataIntialize = function (calendarIndex, monthDetailsJson) {
	this.data[calendarIndex].MONTH = monthDetailsJson.MONTH;
	this.data[calendarIndex].YEAR = monthDetailsJson.YEAR;
	this.data[calendarIndex].calendarDATA = this.retrivemonthData(this.data[calendarIndex].MONTH, this.data[calendarIndex].YEAR, this.labelSkinBelongsINmonth, this.labelSkinNotBelongsINmonth, this.cellSkin, this.cellFocusSkin);
...
AcessByInstance:false
 */
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.updateUiByData = function (data, FLEXINDEX) {
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.updateCellUiByFlexIndex = function (FLEXINDEX, i, data_json) {

	var id = "LABEL" + FLEXINDEX + "INDEX" + i;
	var cell_id = "FLEX_CELL" + FLEXINDEX + "INDEX" + i;
	...
	if (data_json.LABEL) {
		if (data_json.LABEL.focusSkin) {
			if (this[id].focusSkin != data_json.LABEL.focusSkin) {
				this[id].focusSkin = data_json.LABEL.focusSkin;
			}
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.getcalendar = function () {
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.setDataAtIndex = function (calendarIndex, index, data_json) {
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.retrivemonthData = function (month, year, labelSkinBelongsINmonth, labelSkinNotBelongsINmonth, cellSkin,focusSkin) {
	var d = new Date();
	d.setDate(1);
	d.setMonth(month);
	d.setFullYear(year);
	...
	for (var i = 0; i <= emptycellslength - 1; i++) {
		data[i] = {
			"LABEL" : {
                ...
                "focusSkin": labelSkinNotBelongsINmonth
			},
			"FLEX" : {
				"skin" : cellSkin
			},
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.leftShift = function (calendarIndex, monthDetailsJson) {
	...
	var next = kony.apps.ess.myLeaveTabUI.Reusable.calendar.HelperFunction.retriveMonthDetails(this.data[lastshiftIndex].MONTH, this.data[lastshiftIndex].YEAR, 1);
	...
	this.data[insertIndex].calendarDATA = this.retrivemonthData(next.MONTH, next.YEAR, this.labelSkinBelongsINmonth, this.labelSkinNotBelongsINmonth, this.cellSkin,this.cellFocusSkin);
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.rightShift = function (calendarIndex, monthDetailsJson) {
	...
	var prev = kony.apps.ess.myLeaveTabUI.Reusable.calendar.HelperFunction.retriveMonthDetails(this.data[lastshiftIndex].MONTH, this.data[lastshiftIndex].YEAR, -1);
	this.data[insertIndex].MONTH = prev.MONTH;
	this.data[insertIndex].YEAR = prev.YEAR;
	this.data[insertIndex].FLEXINDEX = updateFlexIndex;
	this.data[insertIndex].calendarDATA = this.retrivemonthData(prev.MONTH, prev.YEAR, this.labelSkinBelongsINmonth, this.labelSkinNotBelongsINmonth, this.cellSkin,this.cellFocusSkin);
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.animation = function (calendarWidget, left, Widget_Helper) {
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.updatecalendarRows = function () {
	var middle = parseInt((this.FLEXCOUNT - 1) / 2);
...
AcessByInstance:false
 */
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.updateMonthAndYear = function () {
	var middle = parseInt((this.FLEXCOUNT - 1) / 2);
	this.month = this.data[middle].MONTH;
	this.year = this.data[middle].YEAR;
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.setMonthandYear = function (month_param, year_param) {
	...
	if ((diff >= -1) && (diff <= 1)) {
		...
		if (diff > 1) {
			kony.print("in diff")
			var shiftingmonthdetails = kony.apps.ess.myLeaveTabUI.Reusable.calendar.HelperFunction.retriveMonthDetails(this.month, this.year, (diff - 1));
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.getIndexByDate = function (Date_crossCheck) {
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.getCelldataAtIndex = function (index) {
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.resetCellStatusAtIndex = function (index) {

	var currentDataIndex = parseInt((this.FLEXCOUNT - 1) / 2);
	var CURRENTFLEXINDEX = this.data[currentDataIndex].FLEXINDEX;
	var flex_cell_id = "FLEX_CELL" + CURRENTFLEXINDEX + "INDEX" + index;
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.setCellStatus = function (index, data_json) {
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.SetBlurSkinning = function () {
	...
	for (var calendarIndex = 0; calendarIndex < this.data.length; calendarIndex++) {
		...
		for (var i = 0; i < 42; i++) {
			var flex_cell_id = "FLEX_CELL" + CURRENTFLEXINDEX + "INDEX" + i;
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.ResetBlur = function () {
	for (var calendarIndex = 0; calendarIndex < this.data.length; calendarIndex++) {
		var CURRENTFLEXINDEX = this.data[calendarIndex].FLEXINDEX;
...
kony.apps.ess.myLeaveTabUI.Reusable.calendarWIDGET.prototype.resetTotalcalendarUI = function () {
	for (var calendarIndex = 0; calendarIndex < this.data.length; calendarIndex++) {
		var CURRENTFLEXINDEX = this.data[calendarIndex].FLEXINDEX;
		for (var i = 0; i < 42; i++) {
			var flex_cell_id = "FLEX_CELL" + CURRENTFLEXINDEX + "INDEX" + i;
...
kony.apps.ess.myLeaveTabUI.Reusable.calendar.HelperFunction.retriveMonthDetails = function (month, year, diff) {
...
kony.apps.ess.myLeaveTabUI.Reusable.calendar.HelperFunction.retriveDayName = function (day_param) {
...
kony.apps.ess.myLeaveTabUI.Reusable.calendar.HelperFunction.retriveMonthName = function (month_int) {
