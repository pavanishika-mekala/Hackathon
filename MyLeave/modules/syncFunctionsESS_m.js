...
kony.apps.coe.ess.syncFunctions.prototype.checkConnectivity = function() {
    var config = {};
    //isOnline is response parameter set to true or false wrt to network change
    config.statusChange = function(isOnLine) {
        var offlineObj = new kony.apps.coe.ess.syncFunctions();
    ...
    //Triggered when the network state changes
...
kony.apps.coe.ess.syncFunctions.prototype.encryptDataDW=function(data) {
    var myUniqueKey = kony.crypto.newKey("passphrase", 128, {passphrasetext: ["inputstring1"], subalgo: "aes", passphrasehashalgo: "md5"});
    var properties = {
        padding: "pkcs5",
        mode: "cbc",
        initializationvector: "1234567890123456"
...
kony.apps.coe.ess.syncFunctions.prototype.decryptDataDW=function (data) {
    var myUniqueKey = kony.crypto.newKey("passphrase",128,{passphrasetext: ["inputstring1"],subalgo: "aes",passphrasehashalgo: "md5"});
    var properties = {
        padding: "pkcs5",
        mode: "cbc",
    ...
    var decryptedData = kony.crypto.decrypt("aes", myUniqueKey, data, properties);
    kony.print(decryptedData);
    return decryptedData;
};
