...
kony.apps.coe.Reusable.TimelineCreationDW.prototype.storeCoordinatesOfTimeLineDW = function () {
	...
	timeDuration = kony.store.getItem("minTaskDuration");
  //alert(frmName.timeLineScrollFlex.widgets().length);
	frmName.timeLineScrollFlex.flexSlider.width = kony.apps.generalizeWidthInDp(20) + "dp";
	/*frmName.timeLineScrollFlex.flexSlider.setGestureRecognizer(constants.GESTURE_TYPE_TAP, {
		fingers : 1,
		taps : 2
	},this.TapSlidertoFixTaskDW );*/
	...
	//kony.apps.coe.ess.myTime.navigation.prototype.onClickOfDeleteMenuTab();
...
dragSliderLeftDW = function (eventobject, x, y) {
	...
	var widthOfInnerFlex = parseInt(width) - 25;
...
//     //#ifdef android
		...
		frmName.timeLineScrollFlex.flexSlider.flexSliderLeftPin.onTouchEnd = function () {
			flagToMove=0;
			// frmName.timeLineScrollFlex.enableScrolling = true;
			kony.apps.coe.Reusable.TimelineCreationDW.showTimeOnHead();
		};
		//#endif
...
kony.apps.coe.Reusable.TimelineCreationDW.prototype.fillTimelineDW = function (data) {
	...
	if (data.length > 0) {
		...
		frmName[flexId].skin = "sknflxbg1C7393op100";
...
kony.apps.coe.Reusable.TimelineCreationDW.showTimeOnHead = function(){
	var timeObj = kony.apps.coe.Reusable.TimelineCreationDW.fetchSliderTime();
	frmApplyLeaveDW.lblTimeDuration.text = timeObj.start + " - " + timeObj.end;
	frmApplyLeaveDW.lblHoursDuration.text = "Duration: "+ timeObj.duration+" Hours";
	frmApplyLeaveDW.forceLayout();
}
kony.apps.coe.Reusable.TimelineCreationDW.fetchSliderTime = function(){
	var obj=new kony.apps.coe.Reusable.TimelineCreationDW();
	var frmName=kony.apps.coe.Reusable.TimelineCreationDW.parentWidgetName;
	var XCoordinatesOfTimeLine=kony.apps.coe.Reusable.TimelineCreationDW.XCoordinatesOfTimeLine;
	var startIndex=obj.search(parseInt(frmApplyLeaveDW.flexSlider.left),XCoordinatesOfTimeLine)+1;
	var endIndex=obj.search(parseInt(frmApplyLeaveDW.flexSlider.left)+parseInt(frmApplyLeaveDW.flexSlider.frame.width)-25,XCoordinatesOfTimeLine)+1;
	var duration = (endIndex - startIndex)*15/60;
	startTime=XCoordinatesOfTimeLine[startIndex][1];
	endTime=XCoordinatesOfTimeLine[endIndex][1];
	return {"start": startTime, "end":endTime, "duration": duration, "startIndex": startIndex, "endIndex": endIndex};
}
