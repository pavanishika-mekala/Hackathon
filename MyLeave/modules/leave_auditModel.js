...
// ****************Generated On Mon Dec 11 21:45:46 UTC 2017leave_audit*******************
...
* e.g. com.kony.MYLEAVE.leave_audit.remove("where createdts like 'A%'", successcallback,errorcallback, true);
*************************************************************************************/
...
* e.g. com.kony.MYLEAVE.leave_audit.find("where createdts like 'A%'", successcallback,errorcallback);
*************************************************************************************/
...
com.kony.MYLEAVE.leave_audit.pkCheck = function(pks,wcs,errorcallback,opName){
	...
	if(kony.sync.isNull(pks)){
		sync.log.error("Primary Key id not specified in  " + opName + "  an item in leave_audit");
		kony.sync.verifyAndCallClosure(errorcallback, kony.sync.getErrorTable(kony.sync.errorCodePrimaryKeyNotSpecified,kony.sync.getPrimaryKeyNotSpecifiedMsg("id",opName,"leave_audit")));
		return false;	
	}
...
com.kony.MYLEAVE.leave_audit.validateNull = function (valuestable,errorcallback){
	...
	if(valuestable.leave_id!==undefined){
...
com.kony.MYLEAVE.leave_audit.validateNullInsert = function (valuestable,errorcallback){
	...
	if(kony.sync.isNull(valuestable.id) || kony.sync.isEmptyString(valuestable.id)){
		sync.log.error("Mandatory attribute id is missing for the SyncObject leave_audit.");
		errorcallback(kony.sync.getErrorTable(kony.sync.errorCodeMandatoryAttribute,kony.sync.getErrorMessage(kony.sync.errorCodeMandatoryAttribute, "leave_audit", "id")));
		return false;
	}
	if(kony.sync.isNull(valuestable.leave_id) || kony.sync.isEmptyString(valuestable.leave_id)){
		sync.log.error("Mandatory attribute leave_id is missing for the SyncObject leave_audit.");
