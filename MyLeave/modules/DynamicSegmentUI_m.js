...
kony.apps.ess.DynamicSegment.prototype.setDataAtIndex = function (index, data) {
	...
	for (var i in WidgetsArray) {
		...
		if (this.WidgetDataMap[Propertyid]) {
			...
			kony.apps.coe.ess.Validation.WidgetPropertyBinding(TemplateCell[WidgetId], data[propertyName]);
		} else {
			//Using the same id name for the Widget
			kony.apps.coe.ess.Validation.WidgetPropertyBinding(TemplateCell[WidgetId], data[Propertyid]);
		}
	}
