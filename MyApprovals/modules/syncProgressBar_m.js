...
kony.apps.coe.ess.Sync.UI.getProgressbarUI = function() {
   //forms and their corresponding headers
    var formsAndHeaderHeights={
       "frmApprovalHome":"flxHeader",
       "frmApprovalRequestDetail":"flxHeader",
       "frmAudit":"flxAuditHeader",
       "frmAuditTrail":"flxAuditTrailHeader",
       "frmFullDetails":"flxFullDetailsHeader",
       "frmIsLaterSearch":"flxHeader",
       "frmMultiSelection":"flxHeader",
       "frmNotificationList":"flxHeader",
       "frmPdfReader":"flxHeader",
       "frmRequestedList":"flxHeader",
       "frmSearch":"flxHeader",
       "frmSelect":"flxHeader",
       "frmSettings":"flxHeader",
     };
...
kony.apps.coe.ess.Sync.UI.showInformationToast = function(message) {
    kony.print("-- Start showInformationToast --");
    ...
    kony.print("-- End showInformationToast --");
 ...
 */
kony.apps.coe.ess.Sync.UI.showSyncProgressBar = function(formReference) {
    if (!formReference)
    ...
    formReference.flxSyncProgresssBar.setVisibility(true);
}; //End of showSyncProgressBarToCurrentForm
/**
 * Removes Sync Progressbar from Form. If no form is passed, It will remove from current form, if exists.
 */
kony.apps.coe.ess.Sync.UI.hideSyncProgressBar = function(formReference) {
    if (!formReference) {
        formReference = kony.application.getCurrentForm();
    }
    ...
    if (formReference.flxSyncProgresssBar) {
        ...
        formReference.flxSyncProgresssBar.setVisibility(false);
    }
}; //End of hideSyncProgressBarFrom
...
kony.apps.coe.ess.Sync.UI.setProgressPercentage = function(number) {
    kony.print("-- Start setProgressPercentage --");
    // Input validations. 
    if (number > 100) {
        number = 100;
    }
    ...
    kony.print("-- End setProgressPercentage --");
...
kony.apps.coe.ess.Sync.UI.startSyncProgressBar = function() {
    ...
    kony.apps.coe.ess.Sync.UI.progressPercentage = 0;
...
kony.apps.coe.ess.Sync.UI.showSyncProgressBarIfSyncing = function() {
    ...
    if (kony.apps.coe.ess.Sync.UI.syncingInBackground) {
        kony.apps.coe.ess.Sync.UI.hideSyncProgressBar(kony.apps.coe.ess.Sync.UI.previousForm);
        kony.apps.coe.ess.Sync.UI.showSyncProgressBar();
        ...
        kony.apps.coe.ess.Sync.UI.hideSyncProgressBar();
        if (currentForm.flxSyncToastMessage)
...
kony.apps.coe.ess.Sync.UI.stopSyncProgressBarWithSuccessMessage = function() {
    kony.print("-- Start stopSyncProgressBarWithSuccessMessage");
    kony.apps.coe.ess.Sync.UI.setProgressPercentage(100);
    kony.apps.coe.ess.Sync.UI.stopSyncProgressBar();
    kony.print("-- End stopSyncProgressBarWithSuccessMessage");
};

kony.apps.coe.ess.Sync.UI.stopSyncProgressBarWithErrorMessage = function() {
    kony.apps.coe.ess.Sync.UI.showErrorToast(kony.i18n.getLocalizedString("i18n.ess.syncProgressBar.syncError"), function() {
...
kony.apps.coe.ess.Sync.UI.stopSyncProgressBar = function() {
    try {
        kony.timer.schedule("syncBarRemoveTmr", function() {
            ...
            kony.apps.coe.ess.Sync.UI.progressPercentage = 10;
        }, 3, false);
    } catch (e) {
        kony.print("Ignoring Error: in stopSyncProgressBar : " + JSON.stringify(e));
    }
