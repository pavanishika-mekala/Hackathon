...
function ConfigProviderMFAPPOnline(backendUrl) {
    ...
    function getConfigurationFromBundle(bundleId, configurationObject, successCallback) {
        try {
            ...
            httpclient.open(constants.HTTP_METHOD_GET, encodeURI(backendUrl+"/ConfigurationSDOApprovals/objects/Configurations?$filter=configurationBundle_id eq "+bundleId));
            httpclient.send();
        } catch (e) {
            alert(e);
        }
    ...
    this.getConfigurations = function(contextObject, successCB) {
        var urlForRole = backendUrl+"/ConfigurationSDOApprovals/objects/ConfigurationMaster?$filter=role_id eq " + contextObject.roleId
